"""Bosses' Rise partial reviews; all accepted owner tables stay immutable."""
...
def save_section(d,stem,title):
    ...
    r.update(checkpoint=d['checkpoint'],scope=d['scope'],notes_file=stem+'.json',exact_next_task=d['exact_next_task']);write_json(p,r);refresh(d['checkpoint'])
    for f,_ in VIEWS:
    ...
    o=read_json(OUT/'research-decision.json');o.update(checkpoint=d['checkpoint'],next_task=d['exact_next_task'],latest_bossesrise_decision=d['status'],save_mode=False,save_reason=None);o['checkpoints'][d['previous_checkpoint']]=d['starting_sha'];o['checkpoints'][d['checkpoint']]='Self: exact SHA from commit/live verification.';write_json(OUT/'research-decision.json',o)
    lines=['# '+title,'',d['scope'],'','Static review only. Future runtime fixtures remain unexecuted; whole Bosses’ Rise review is PARTIAL.','']
