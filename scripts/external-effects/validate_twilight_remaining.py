...
def validate_remaining():
    ...
    for sp in sorted((OUT/'semantic-sections').glob('twilightforest-*.json')):
        ...
        if d['slug']=='passive-entities':
            from validate_twilight_passive_entities import validate_passives
            validate_passives(d,s,old,new,methods,ins,pos,target)
        if d['slug']=='contact-hazards':
            from validate_twilight_contact_hazards import validate_contacts
            validate_contacts(d,s,old,new,methods,ins,pos,target)
        if d['slug']=='mechanical-hazards':
            from validate_twilight_mechanical_hazards import validate_mechanical
            validate_mechanical(d,s,old,new,methods,ins,pos,target)
        if d['slug']=='ominous-progression':
            from validate_twilight_ominous_progression import validate_ominous
            validate_ominous(d,s,old,new,methods,ins,pos,target)
