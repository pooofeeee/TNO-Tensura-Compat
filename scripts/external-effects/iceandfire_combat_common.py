...
def save_section(d,stem,title):
    ...
    r=read_json(OUT/'mod-reviews/iceandfire.json');r.update(checkpoint=d['checkpoint'],scope=d['scope'],notes_file=stem+'.json',exact_next_task=d['exact_next_task']);write_json(OUT/'mod-reviews/iceandfire.json',r);refresh(d['checkpoint'])
    for f,_ in VIEWS:
        o=read_json(OUT/f);o.update(checkpoint=d['checkpoint'],unfinished_review=dict(mod_key='iceandfire',status='PARTIAL',promoted_records=0,notes_file=stem+'.json'));write_json(OUT/f,o)
    o=read_json(OUT/'research-decision.json');o.update(checkpoint=d['checkpoint'],next_task=d['exact_next_task'],latest_iceandfire_subsection_decision=d['status'],save_mode=False,save_reason=None);o['checkpoints'][d['previous_checkpoint']]=d['starting_sha'];o['checkpoints'][d['checkpoint']]='Self: exact SHA from commit/live verification.';write_json(OUT/'research-decision.json',o)
    lines=['# '+title,'',d['scope'],'','Static subsection complete. Runtime fixtures unexecuted; no Stage or production implementation.','']
    for k,v in d['facts'].items():lines+=['## '+k.replace('_',' ').capitalize(),'',v,'']
    lines+=['## TNO integration decisions','']
    for m in d['mechanic_packages']:lines += ['- **'+m['name']+'**: '+', '.join(m['tno_categories'])+'. Stage: '+('yes; '+m['single_scaling_point'] if m['stage_scaling_needed'] else 'no additional value; '+m['stage_reason'])]
    lines+=['','[Machine-readable packages, native paths and future fixtures]('+stem+'.json). Validation reproduces new witnesses, checks significant call order/amounts, preserves accepted records and prior evidence, runs five tooling tests and diff checks. No whole-mod completion claim.','','Exact next task: '+d['exact_next_task'],'']
...
def preserve_section(d):
    for r in d['reference_files']:assert sha256(OUT/r['file'])==r['sha256']
