param([string]$EvidencePath='docs/benchmarks/phase6-energy-steal-physical-prerequisite/es3b-runtime.jsonl',[string]$OutputPath)
$ErrorActionPreference='Stop';$extractor=Join-Path $PSScriptRoot 'extract-phase6-energy-historical.ps1'
...
$mutations=[ordered]@{
    ...
    forged_energy_callback_on_plain={param($r) $r[16].traces+=@($r[2].traces|Where-Object boundary -eq energy_callback)}
...
$mutations['false_invulnerability_at_attempt']={param($r) ($r[16].traces|Where-Object boundary -eq physical_attempt).cooldown=0}
$mutations['false_target_clock']={param($r) ($r[16].traces|Where-Object boundary -eq physical_attempt).target_tick+=20}
$mutations['false_world_empty_time']={param($r) ($r[16].traces|Where-Object boundary -eq physical_attempt).world_empty_time=0}
$mutations['hurt_time_decay_fabricated']={param($r) ($r[16].traces|Where-Object boundary -eq physical_return).hurt_time=0}
$mutations['last_hurt_forged']={param($r) ($r[16].traces|Where-Object boundary -eq physical_attempt).last_hurt=0}
$mutations['incoming_cancelled_instead_of_iframe']={param($r) ($r[16].traces|Where-Object boundary -eq incoming_lowest).result=$false}
$mutations['rejected_hurt_forged_true']={param($r) ($r[16].traces|Where-Object boundary -eq physical_return).result=$true}
$mutations['phantom_adaptive_increment']={param($r) $r[16].post.adaptive.counts.arrow++}
$mutations['missing_server_tick']={param($r) $r[16].traces=@($r[16].traces|Where-Object {!($_.boundary -eq 'server_tick' -and $_.relative_tick -eq 10)})}
$mutations['source_cooldown_bypass']={param($r) ($r[16].traces|Where-Object boundary -eq physical_attempt).tags+='minecraft:bypasses_cooldown'}
$mutations['SHP_subtraction']={param($r) $r[16].post.shp-=1}
$results=@()
...
$report=[ordered]@{schema='tno.phase6.energy_native_path.extractor_tests.v1';checkpoint='ES3B';status='PASS';negative_tests=$results.Count;results=$results}
